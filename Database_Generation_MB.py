...
#
# Mike Bonheure  - Thesis Louis Boucqué
# V1: 12/11/25
# Modified: 12/11/25
# =========================
...
if __name__ == "__main__":
    ...
    T_in_list      = [650.0 + 273.15]                     # K
    P_in_list      = [1.5e5]                               # Pa
    X_H2O_list     = [0.30]                                 # /
    L_list         = [6.5, 8.5]                           # m (om onze mean RTD te bepalen gaan we lengte variëren)
    H_peak_list    = [2.5e6]                       # W/m^2 (non-negative only)
    ...
    CSV_Database = True
    N_threads    = min(1000, os.cpu_count() or 4, len(cases))
    ...
    final_csv     = "Database_FINAL.csv" if CSV_Database else None
