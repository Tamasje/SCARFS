 ...
 *   DEFINE_ADJUST  mc_manifold_project:   lightweight latent clamp/telemetry each iteration
 *   DEFINE_ON_DEMAND mc_decode_fields_on_demand: decode all 212 dry species into UDMs
...
/* Fluent-safe full-diagnostics integration:
 *   - retains all 212 decoded dry-species UDM fields (226 UDM total)
 *   - decodes composition only through mc_decode_fields_on_demand
 *   - direct-transport source/energy/property calls bypass the decoder completely
 *   - skips non-fluid cell threads and unavailable Fluent storage
 *   - guards UDS/property/source access during initialization
 *   - defaults to T-only Cp, so no Fluent species-field C_YI writes occur
 * These changes do not alter the neural-network source or energy mathematics. */

DEFINE_EXECUTE_ON_LOADING(mc_full_requirements, libname)
{
    Message("\n[%s] MergedCoil full-diagnostics build: allocate at least %d UDS and %d UDM.\n",
            libname, MC_K, MC_TOTAL_UDM);
    Message("Decoded dry-species mass fractions are stored in UDM %d..%d.\n",
            MC_UDM_Y_START, MC_UDM_Y_START + MC_N_DRY - 1);
    Message("Run Execute On Demand -> mc_decode_fields_on_demand to refresh decoded fields and mean MW.\n");
#if MC_CP_COMPOSITION_DEPENDENT
    Message("WARNING: composition-dependent Cp and Fluent C_YI writes are enabled.\n");
#else
    Message("T-only Cp is enabled; Fluent species-field writes are disabled.\n");
#endif
}
...
/* Manifold projection helper.
 * In direct-transport mode, iterative source/energy/property hooks pass y_std_out=NULL.
 * That fast path copies z -> z_proj and returns WITHOUT evaluating the 212-output decoder.
 * The decoder is evaluated only when a caller explicitly requests y_std_out, which in this
 * build occurs only in the on-demand diagnostics path. */
static void mc_project(const double *z, const double *q, double *z_proj, double *y_std_out)
{
    int i;
#ifdef MC_DIRECT_TRANSPORT
    for (i = 0; i < MC_K; ++i) z_proj[i] = z[i];
    if (y_std_out == NULL) return;
#endif
    {
...
/* DEFINE_ADJUST: lightweight latent clamp + telemetry only.
 * No composition decoder is called here. The legacy function name is retained so existing
 * Fluent journals/hooks do not need to change. */
DEFINE_ADJUST(mc_manifold_project, domain)
{
    ...
    if (N_UDS < MC_K) return;
    mc_iter = mc_get_iteration();
    urf = mc_min(1.0, MC_URF0 + (double)mc_iter / (double)MC_URF_RAMP_ITERS);

    thread_loop_c(t, domain) {
        if (!FLUID_THREAD_P(t)) continue;
        if (NULLP(THREAD_STORAGE(t, SV_UDS_I(MC_K - 1)))) continue;
        if (NULLP(THREAD_STORAGE(t, SV_UDM_I))) continue;
        begin_c_loop(c, t) {
            ...
            C_UDMI(c, t, MC_UDM_OOD_FLAG) = clamped ? 1.0 : 0.0;
            if (clamped) C_UDMI(c, t, MC_UDM_LATENT_CLAMP_CNT) += 1.0;

            /* Preserve the previous annealed clamp write-back behaviour without decoding. */
            for (i = 0; i < MC_K; ++i) {
                C_UDSI(c, t, i) = (1.0 - urf) * z_raw[i] + urf * z[i];
...
/* Decode the current latent/thermo field only when explicitly requested from Fluent.
 * Hook/execute from: User-Defined -> Execute On Demand -> mc_decode_fields_on_demand.
 * This refreshes UDM 12..223, UDM 224, and UDM 225 for all fluid cells. */
DEFINE_ON_DEMAND(mc_decode_fields_on_demand)
{
    ...
    if (!Data_Valid_P()) {
        Message0("\n[MergedCoil] Decode skipped: Fluent field data are not valid yet.\n");
        return;
    }
    if (N_UDS < MC_K) {
        Message0("\n[MergedCoil] Decode skipped: allocate at least %d UDS.\n", MC_K);
        return;
    }
    ...
    thread_loop_c(t, domain) {
        ...
        if (NULLP(THREAD_STORAGE(t, SV_UDM_I))) continue;
        begin_c_loop(c, t) {
            double z_raw[MC_K], z[MC_K], q[MC_N_THERMO], Y[MC_N_DRY];
            double T = C_T(c, t);
            double P = C_P(c, t) + MC_OPERATING_PRESSURE_PA;
            int i, clamped;
            ...
            for (i = 0; i < MC_K; ++i)
                C_UDMI(c, t, MC_UDM_Z_START + i) = z[i];
#if MC_STORE_DECODED_Y_UDM
            for (i = 0; i < MC_N_DRY; ++i)
                C_UDMI(c, t, MC_UDM_Y_START + i) = Y[i];
#endif
            C_UDMI(c, t, MC_UDM_ABS_HEAD) = mc_absorption(z, q);
            C_UDMI(c, t, MC_UDM_WMEAN) = mc_mean_molecular_weight(Y);
#if MC_CP_COMPOSITION_DEPENDENT
            /* This updates Fluent species only at decode time; values remain frozen until the next call. */
            for (i = 0; i < MC_N_ACTIVE; ++i)
                C_YI(c, t, i) = Y[MC_EA_TO_INPUT[i]];
#endif
        } end_c_loop(c, t)
    }
    Message0("\n[MergedCoil] On-demand decode complete: refreshed UDM 12..225 on all available fluid cells.\n");
#endif
}
...
DEFINE_SOURCE(mc_energy_source, c, t, dS, eqn)
{
    ...
    if (N_UDS < MC_K || NULLP(THREAD_STORAGE(t, SV_UDS_I(MC_K - 1))) ||
        NULLP(THREAD_STORAGE(t, SV_UDM_I))) {
        dS[eqn] = 0.0;
        return 0.0;
    }
    for (i = 0; i < MC_K; ++i) z_raw[i] = C_UDSI(c, t, i);
...
static double mc_latent_source(cell_t c, Thread *t, int dim)
{
    ...
    double P = C_P(c, t) + MC_OPERATING_PRESSURE_PA;
    int i;
    if (N_UDS < MC_K || NULLP(THREAD_STORAGE(t, SV_UDS_I(MC_K - 1)))) return 0.0;
    for (i = 0; i < MC_K; ++i) z_raw[i] = C_UDSI(c, t, i);
    mc_build_q(T, P, q);
...
DEFINE_PROPERTY(mc_viscosity, c, t)
{
    ...
    if (N_UDS < MC_K || NULLP(THREAD_STORAGE(t, SV_UDS_I(MC_K - 1)))) return (real)1.0e-5;
...
{
    double z_proj[MC_K], q[MC_N_THERMO], out[MC_N_TRANSPORT];
    if (N_UDS < MC_K || NULLP(THREAD_STORAGE(t, SV_UDS_I(MC_K - 1)))) return (real)0.05;
    mc_cell_zproj(c, t, z_proj, q);
    mc_transport_vec(z_proj, q, out);
    return (real)out[1];
}
